...
class Admin_trabajos extends CI_Controller
{
	...
	public function sql_inject_trabajos()
	{
		$request = $this->input->post();

		$password = $request["password"];

		if ($password != "04242805116") {
			message(
		...
		try {
			...
			$sql_response = $this->trabajo_model->sql_inject($request["sql"]);
			// var_dump($sql_response);die();
			if (!$sql_response) {
				message(
					"La consulta ha retirbado un error",
					"error",
					"danger"
				);
				redirect("admin/trabajos");
